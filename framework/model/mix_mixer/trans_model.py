...
from framework.model.mix_mixer.hidden_mix.trans_mix import BalancedTransMix, SoftTransMix, HardTransMix
...
class MLPMixerVQASystem(nn.Module):
    def __init__(self, depth=12, emb_size=768, drop_out=.2, qus_embedding_dim=300, num_patch=216,
                 ...
                 mixup_alpha=5, mixup_beta=1, prob=1, word_size=305, select_hidden_mix="hard_trans_mix"):
...
def trans_mlp_mixer_small(**kwargs):
...
def trans_mlp_mixer_base(**kwargs):
    model = MLPMixerVQASystem(
...
def trans_mlp_mixer_large(**kwargs):
    model = MLPMixerVQASystem(
...
def trans_mlp_mixer_huge(**kwargs):
    model = MLPMixerVQASystem(
