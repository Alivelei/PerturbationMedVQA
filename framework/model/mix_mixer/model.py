...
class MLPMixerVQASystem(nn.Module):
    ...
    def forward(self, img, qus):
        img_tokens = self.img_to_tokens(img)
        ...
        all_features = torch.cat([img_tokens, qus_embedding], dim=1)
...
def general_mlp_mixer_small(**kwargs):
...
def general_mlp_mixer_base(**kwargs):
    model = MLPMixerVQASystem(
...
def general_mlp_mixer_large(**kwargs):
    model = MLPMixerVQASystem(
...
def general_mlp_mixer_huge(**kwargs):
    model = MLPMixerVQASystem(
