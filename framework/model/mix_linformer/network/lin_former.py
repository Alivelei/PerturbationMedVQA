...
if __name__ == '__main__':
    model = nn.ModuleList([Linformer(
        ...
        one_kv_head=True,
        share_kv=True
    ) for _ in range(12)]).cuda()

    x = torch.randn(2, 218, 768).cuda()
    for layer in model:
        out = layer(x)  # (1, 4096, 512)
    print(out.shape)
    print(sum(x.numel() for x in model.parameters()))  # 72130560
