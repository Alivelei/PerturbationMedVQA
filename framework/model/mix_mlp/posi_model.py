...
from framework.model.mix_mlp.network.mlpres import MLPFeedForward, ClassificationHead
from framework.model.mix_mlp.network.word_embedding import WordEmbedding
from framework.model.mix_mlp.hidden_mix.rand_replace_zero import RandReplaceZeroMix
from framework.model.mix_mlp.hidden_mix.rand_gaoss_mix import BalancedRandGaossMix, HardRandGaossMix, SoftRandGaossMix
from framework.model.mix_mlp.hidden_mix.content_replace_mix import RandContentReplaceMix
...
class MLPVQASystem(nn.Module):
    def __init__(self, depth=16, emb_size=768, drop_out=.3, qus_embedding_dim=300, hidden_size=1024, ans_size=223,
                 ...
                 word_size=305, select_hidden_mix="hard_rand_gauss_mix"):
...
def posi_mlp_struc_small(**kwargs):
    model = MLPVQASystem(
...
def posi_mlp_struc_base(**kwargs):
    model = MLPVQASystem(
...
def posi_mlp_struc_large(**kwargs):
    model = MLPVQASystem(
...
def posi_mlp_struc_huge(**kwargs):
    model = MLPVQASystem(
...
if __name__ == '__main__':
    ...
    res = model(a, b, "train")
    t2 = time.time()
    print(t2 - t1)  # rand_content 0.24407   rand_zero 0.2437   rand_gauss 0.24722
    print(res.shape)
