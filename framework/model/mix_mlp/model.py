...
class MLPVQASystem(nn.Module):
    def __init__(self, depth=24, emb_size=1024, drop_out=.3, qus_embedding_dim=300, hidden_size=1536, ans_size=223,
...
def general_mlp_struc_small(**kwargs):
    model = MLPVQASystem(
...
def general_mlp_struc_base(**kwargs):
    model = MLPVQASystem(
...
def general_mlp_struc_large(**kwargs):
    model = MLPVQASystem(
...
def general_mlp_struc_huge(**kwargs):
    model = MLPVQASystem(
...
if __name__ == '__main__':
    ...
    t2 = time.time()
    print(t2 - t1)
    print(res.shape, m_l.shape)  # 0.480711
    print(sum(x.numel() for x in model.parameters()))  # 26611147
